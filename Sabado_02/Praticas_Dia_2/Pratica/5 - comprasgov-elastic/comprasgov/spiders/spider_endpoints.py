...
class Endpoints(scrapy.Spider):
    ...
    name = "comprasgov-endpoints"

    # Define as configurações para esse spider (output, etc...)
    custom_settings = {
        "ITEM_PIPELINES" : {
            'comprasgov.pipelines.CSVEndpointsPipeline': 200,  #300
        }
    }
    ...
    def start_requests(self):
        ...
        url = "http://compras.dados.gov.br/docs/swagger.html"
    ...
    def parse_methods_json(self, response):
        ...
        for path in paths:
            ...
            if "resourcePath" in content:
                ...
                url = "http://compras.dados.gov.br/" + response.meta["module_path"] + path + ".json"
    ...
    def parse_methods_html(self, response):
        ...
        for item in table:
            ...
            url = "http://compras.dados.gov.br/" + path.replace(".html", ".json")
